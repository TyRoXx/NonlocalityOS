...
async fn effect() {
    ...
    ));
    {
        ...
        main_lambda_expression
            ...
            .unwrap();
        assert_eq!(concat!(
            "$env={literal(DeepTree { blob: TreeBlob { content: b\"\" }, references: DeepTreeChildren { references: [] } })}($arg) =>\n",
            "  [literal(DeepTree { blob: TreeBlob { content: b\"\" }, references: DeepTreeChildren { references: [DeepTree { blob: TreeBlob { content: b\"Hello, \" }, references: DeepTreeChildren { references: [] } }] } }), $env={literal(DeepTree { blob: TreeBlob { content: b\"\" }, references: DeepTreeChildren { references: [] } })}($arg) =>\n",
            "    [$env, ], ]"),
